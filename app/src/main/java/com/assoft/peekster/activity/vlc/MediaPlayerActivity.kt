...
import com.assoft.peekster.databinding.ActivityVideoPlayerBinding
import com.assoft.peekster.util.contentView
import com.assoft.peekster.util.ext.dpToPx
...
class MediaPlayerActivity : Activity(), IVLCVout.Callback,
    SeekBar.OnSeekBarChangeListener {

    /** Internal variable for obtaining the [ActivityVideoPlayerBinding] binding. */
    private val binding: ActivityVideoPlayerBinding by contentView(R.layout.activity_video_player)
    ...
    private lateinit var allMovie: List<Video>
    ...
    private var movieData: Video? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        selectedMovieId = movieData?.id?.toInt()!!

        allMovie = MediaSource.withVideoContext(this)
            ?.getAllMovieContent(GetVideoContent.externalContentUri) as List<Video>
        ...
        binding.apply {
            this.video = movieData
            lifecycleOwner = this@MediaPlayerActivity
    ...
    private fun nextMovie() {
        if (selectedMovieId < allMovie.size - 1) {
            if (mediaPlayer.isPlaying) {
                releasePlayer()
                handler.removeCallbacks(progressHandler)
            }
            releasePlayer()
            val nextMovie = allMovie[++selectedMovieId]
            binding.video = nextMovie
            binding.executePendingBindings()
            createMediaPlayer(nextMovie)
        }
    }
    ...
    private fun previousMovie() {
        if (selectedMovieId > 0) {
            ...
            releasePlayer()
            val prevMovie = allMovie[--selectedMovieId]
            binding.video = prevMovie
            binding.executePendingBindings()
            createMediaPlayer(prevMovie)
        }
    }
    ...
    private fun createMediaPlayer(video: Video?) {
        try {
            // Create LibVLC
            ...
            media = Media(libvlc, video?.path)
            mediaPlayer.media = media

            binding.seekBar.max = video?.duration!! / 1000
    ...
    private fun releasePlayer() {
        ...
        vOut.removeCallback(this@MediaPlayerActivity)
        vOut.detachViews()
