...
class ShareActivity : Activity() {
     ...
     * @return list of URIs
     */
    private fun buildUriList(): ArrayList<Uri>? {
        return when (intent.action) {
            ...
            ACTION_SEND_SINGLE -> {
                val uriList =
                    ArrayList<Uri>()
                uriList.add(intent.getParcelableExtra(Intent.EXTRA_STREAM) as Uri)
                uriList
            }
            else -> ArrayList()
        }
        ...
        if (isValidIntent()) {
            ...
            listView.onItemClickListener =
                OnItemClickListener { _, _, position, _ ->
                    ...
                    val startTransfer =
                        Intent(this@ShareActivity, TransferService::class.java)
                    startTransfer.action = TransferService.ACTION_START_TRANSFER
                    startTransfer.putExtra(TransferService.EXTRA_DEVICE_ID, device)
                    startTransfer.putParcelableArrayListExtra(TransferService.EXTRA_URIS, uriList)
                    startService(startTransfer)

                    // Close the activity
                    this@ShareActivity.setResult(RESULT_OK)
                    finish()
                }
        }
    }
