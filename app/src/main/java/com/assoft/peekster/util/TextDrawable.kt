...
class TextDrawable private constructor(builder: Builder) :
    ShapeDrawable(builder.mShape) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        val fontSize =
            if (mFontSize < 0) Math.min(width, height) / 2 else mFontSize
    ...
    init {
        ...
        // text and color
        var processedText =
            if (builder.mToUpperCase) builder.mText.toUpperCase(Locale.getDefault()) else builder.mText
        if (builder.mTextMaxLength >= 0) {
            processedText = (when {
                builder.mFirstLetters -> TextUtils.getLetters(
                    processedText,
                    builder.mTextMaxLength
                )
                processedText.length > builder.mTextMaxLength -> processedText.substring(
                    0,
                    builder.mTextMaxLength
                )
                else -> processedText
